...
interface ReactHookFormDemoProps {
    onFilesUploaded: (file: string) => void;
}

export function ReactHookFormDemo({ onFilesUploaded }: ReactHookFormDemoProps) {
    ...
    React.useEffect(() => {
        if (uploadedFiles.length > 0) {
            onFilesUploaded(uploadedFiles[0].url);
    ...
    return (
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="flex w-full flex-col gap-6">
                ...
                />
                <Button className="w-fit bg-primary text-white" disabled={loading}>
                    Upload
                </Button>
            </form>
        </Form>
