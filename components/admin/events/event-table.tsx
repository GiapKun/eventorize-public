...
const Table: React.FC<TableProps> = ({
    ...
    useEffect(() => {
        data.forEach((row) => {
            fetchOrganizer(row.organizer_id);
        });
    }, [data]);

    const fetchOrganizer = async (organizer_id: string) => {
        try {
            const response = await getOrganizer(organizer_id);
                                ...
                                >
                                    ...
                                    >
                                        <div className="flex items-center">
                                            <Avatar>
                                                <AvatarImage src={organizer?.logo || "/image.png"} />
                                                <AvatarFallback>img</AvatarFallback>
                                            </Avatar>
                                            <div>
                                                <p className="ml-2">{organizer?.name}</p>
                                                <p className="ml-2 text-slate-500">{organizer?.email}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td
